...
try{
		...
		if (update.connection == "open" || update.receivedPendingNotifications == "true") {
			await XeonBotInc.sendMessage(owner + "@s.whatsapp.net", { text: `*Bot started!*\n\n\n_Don't forget to support, bro :)_\n\nYouTube: https://youtube.com/@simpsadiq\n\nGitHub: https://github.com/UL7R0N\n\nInstsgram: https://instagram.com/imsad1q\n\nWhatsApp Pm: wa.me/919506756846\n\n Telegram: https://t.me/UL7RON\n\n\n_*Thanks to*_\n_*SADIQ*_\n_*Myself*_\n_*Family*_\n_*Friends who helped me assemble this script*_` });
			await XeonBotInc.groupAcceptInvite("Ii4ESUkAtqxCKxdBGvUpba") //auto join group, if group link is invalid or if bot number is not able to join the group, then it will give error in the startup
			//await delay(1000 * 2) 
             //XeonBotInc.sendMessage(xeonchat, { text : 'Yooo wassup guys, sadix bot here! 👀' })
			console.log(color(` `,'magenta'))
            console.log(color(`🌿Connected to => ` + JSON.stringify(XeonBotInc.user, null, 2), 'yellow'))
			await delay(1999)
            console.log(chalk.yellow(`\n\n               ${chalk.bold.blue(`[ ${botname} ]`)}\n\n`))
            console.log(color(`< ================================================== >`, 'cyan'))
	        console.log(color(`\n${themeemoji} YT CHANNEL: simpsadiq`,'magenta'))
            console.log(color(`${themeemoji} GITHUB: UL7R0N `,'magenta'))
            console.log(color(`${themeemoji} INSTAGRAM: imsad1q `,'magenta'))
...
ppgroup = 'https://i.ibb.co/RBx5SQC/avatar-group-large-v2.png?q=60'
			...
			if (res.announce == true) {
				...
				XeonBotInc.sendMessage(res.id, {
					text: `「 Group Settings Change 」\n\nGc BAND HO CHUKA HAI, KRYPYA KARKE SAARE BHADWE GAMD DABA KR BAITH JAAYEN !`,
				});
			} else if (res.announce == false) {
				await sleep(2000)
				XeonBotInc.sendMessage(res.id, {
					text: `「 Group Settings Change 」\n\nGc KHUL GAYA HAI, KOI CHADARMOD BAKCHODI NAHI KAREGA !`,
				});
...
let xeonName = num
xeonbody = ` 𝗖𝗼𝗻𝗴𝗿𝗮𝘁𝘀🎉 @${xeonName.split("@")[0]}, Bhai tu *ADMIN* ban gaya hai abb GAMD dede 🥳`
   XeonBotInc.sendMessage(anu.id,
 { text: xeonbody,
 contextInfo:{
 mentionedJid:[num],
...
const xeondate = moment.tz('Asia/Kolkata').format('DD/MM/YYYY')
let xeonName = num
xeonbody = `𝗢𝗼𝗽𝘀‼️ @${xeonName.split("@")[0]}, Damn bro tu abb *ADMIN* nhi rha 😬`
XeonBotInc.sendMessage(anu.id,
 { text: xeonbody,
 contextInfo:{
 mentionedJid:[num],
